...
- **Output layout.** Screenshots are numbered: `/output/<Title (Year)>/1.png`, `2.jpg`, ... for movies and `/output/<Show (Year)>/S01E02/1.png` for episodes; the next number is always one above the highest in the folder. Clips keep their time range: `/output/<Title (Year)>/<Title (Year)> - <in> to <out>.mp4` (episodes add ` - S01E02`). Unicode titles are kept as they are. The strip under the timeline shows the numbered screenshots in order, then clips; drag a screenshot to change its position and the files are renumbered to match (the leftmost is 1). Each tile has download and delete buttons.
